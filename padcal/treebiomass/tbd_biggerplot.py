...
def tbd_biggerplot_exec():
	...
	with open('./inputs/treeAndRoot/biggerTreeAndRoot.csv', 'r+') as in_csv:
			...
			if int(row[1])!=0:
				...
				if conifer in commonName:
					...
					if (dbh < 60): 
						...
						tbd = (42.69 - 12.8 * dbh) + ( 1.242 * pow(dbh,2))
	...
	with open('./outputs/percomponent/treeandRootBiomass_summary_biggerplot.csv' , 'w+') as summary_out_csv:
		...
		summarywriter.writerow(['BIG PLOT #','TBD (Mg ha^-1)','TREE CARBON STOCK (Mg ha^-1)','TREE CO2 (Mg ha^-1)','ROOT BIOMASS (Mg ha^-1)','ROOT CARBON STOCK (Mg ha^-1)','ROOT CO2 (Mg ha^-1)'])
	...
	with open('./outputs/percomponent/treeBiomass_summary_biggerplot.csv' , 'w+') as summary_out_csv:
		summarywriter = csv.writer(summary_out_csv, delimiter=',',lineterminator='\n',quoting=csv.QUOTE_NONE)
		summarywriter.writerow(['PROJECT:',project,'ENCODER:',encoder,'DATE:', date])
		summarywriter.writerow([])
	...
	with open('./outputs/percomponent/rootBiomass_summary_biggerplot.csv' , 'w+') as summary_out_csv:
		summarywriter = csv.writer(summary_out_csv, delimiter=',',lineterminator='\n',quoting=csv.QUOTE_NONE)
		summarywriter.writerow(['PROJECT:',project,'ENCODER:',encoder,'DATE:', date])
		summarywriter.writerow([])
